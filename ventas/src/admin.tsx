...
import { BarDetailPopup, BubbleChart, CollapsibleSection, DonutChart, FunnelChart, Gauge, Info, LlamadasBar, MiniAreaChart, StackedBar, activar, useEscape, useOutside, type DetRow } from './components'
      ...
      <CollapsibleSection title="Venta" defaultOpen>
        <div className="two">
          ...
          <div className="tiles2">
            ...
            <div className="tile t3"><div className="n">{fmtMoney(monto)}</div><div className="l">Monto cerrado (presupuesto)</div></div>
            ...
            {filas.map((f) => {
              ...
              return (
                <tr key={f.u.id} className="row" tabIndex={0} aria-haspopup="dialog" aria-label={`Ver resumen de ${f.u.nombre}`}
                  ...
                  <td><div className="who"><div className={avatarCls(f.u)} title={subAsesor(corte, f.u)} aria-hidden="true">{iniciales(f.u.nombre)}</div><div><div className="nm">{f.u.nombre}</div><div className="sub">{f.ventas} ventas · meta {f.meta ?? '—'}</div></div></div></td>
  ...
  return (
    <div className="popup" ref={ref} style={{ left: x, top: y }} role="dialog" aria-label={`Resumen de ${fila.u.nombre}`}>
      ...
      <div className="pleads">
        ...
        <div className="donut-wrap"><Gauge pct={cumpl} label="meta" size={110} />{cumpl == null && <span className="small muted">sin meta</span>}</div>
      </div>
  ...
  return (
    <>
      ...
      <div className="kgrid">
        ...
        <div className="kcard k2"><div className="l">Conversión<Info termino="Conversión" /></div><div className="n">{leads.length ? pct(ventas.length, leads.length) + '%' : '—'}</div><div className="small muted">{ventas.length} ventas / {leads.length} leads asignados en el rango</div></div>
        <div className="kcard k3"><div className="l">Cumplimiento<Info termino="Cumplimiento" /></div><div style={{ display: 'grid', gridTemplateColumns: '1fr auto', alignItems: 'center', gap: 8 }}><div className="n">{meta ? pct(ventas.length, meta) + '%' : '—'}</div><Gauge pct={meta ? pct(ventas.length, meta) : null} label="meta" size={120} color="var(--c2)" /></div><div className="small muted">{meta ? `meta ${meta}` : 'sin meta configurada (VENTAS_METAS)'}</div></div>
        <div className="kcard k4"><div className="l">Monto total</div><div className="n" style={{ fontSize: 28 }}>{fmtMXN(monto)}</div><div className="small muted">presupuesto de las ventas cerradas</div></div>
      </div>
